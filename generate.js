...
const categories = {
    'casual': { title: '休閒運動類', file: 'casual.html' },
    'formal': { title: '正式商務類', file: 'formal.html' },
...
for (const [catId, catInfo] of Object.entries(categories)) {
    ...
    let html = getHeader(catInfo.title) + `
    <main>
        <div class="container animate-fade-in">
            <h1 class="section-title">${catInfo.title}</h1>
            <div class="product-grid">
`;
    for (const p of catProducts) {
        html += `
                <a href="product-${p.id}.html" class="product-card">
                    <img src="https://loremflickr.com/800/600/${p.img}" alt="${p.name}" class="product-img">
                    <div class="product-info">
                        <span class="product-category">${catInfo.title}</span>
...
${footerHtml}`;
    fs.writeFileSync(catInfo.file, html);
}

// Generate Product Detail Pages
for (const p of products) {
    const catTitle = categories[p.cat].title;
    const html = getHeader(p.name) + `
    <main>
        <div class="container animate-fade-in">
            <div class="product-detail-container">
                <div class="product-gallery">
                    <img src="https://loremflickr.com/800/600/${p.img}" alt="${p.name}">
                </div>
...
const contactHtml = getHeader('聯絡我們') + `
    <main>
        <div class="container animate-fade-in">
